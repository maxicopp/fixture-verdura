...
export async function GET() {
  ...
  if (!tournament) {
    ...
    return Response.json({
      ...
      exists: false,
      context: {
        leagueChampion: (lastLeague?.champion as string) || null,
        leagueSeason: (lastLeague?.season as string) || null,
        copaChampion: (lastCopa?.champion as string) || null,
        copaSeason: (lastCopa?.season as string) || null,
        copaStatus: (lastCopa?.status as string) || null,
      },
    }, { status: 404 })
  }
  ...
  const match = await dbGet(
    ...
    [tournament.id as number, 'recopa-final']
  )
  ...
  if (match) {
    matchData = {
      ...
      played: !!match.played,
      penaltyWinner: (match.penalty_winner as string) ?? null,
      homePenalties: (match.home_penalties as number) ?? null,
      awayPenalties: (match.away_penalties as number) ?? null,
    }
  }

  const champion = (tournament.champion as string) || null
