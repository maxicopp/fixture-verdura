...
export default function HallOfFame() {
  ...
  return (
    <div className="hall-of-fame">
      ...
      <div className="hall-section">
        <h3 className="hall-section-title"><span>👑</span> Palmarés</h3>
        <div className="palmares-grid">
          {titleCounts.map((player, i) => (
            <div key={player.name} className={`palmares-card ${i === 0 ? 'palmares-leader' : ''}`}>
              ...
              <img src={`/players/${player.name.toLowerCase()}.png`} alt={player.name} className="palmares-avatar" />
              <div className="palmares-info">
                ...
                <span className="palmares-titles">
                  ...
                  <span className="palmares-stars">{'⭐'.repeat(Math.min(player.titles, 5))}</span>
                </span>
              </div>
            </div>
          ))}
      ...
      <div className="hall-section">
        <h3 className="hall-section-title"><span>🏆</span> Historial de Campeones</h3>
            ...
            <div key={t.id} className={`history-item ${i === 0 ? 'history-latest' : ''}`}>
              ...
              <div className="history-content">
                <div className="history-champion-row">
                  <img src={`/players/${t.champion.toLowerCase()}.png`} alt={t.champion} className="history-avatar" />
      ...
      <div className="hall-section">
        <h3 className="hall-section-title"><span>⚽</span> Goleadores Históricos</h3>
          ...
          {allTimeScorers.slice(0, 6).map((scorer, i) => {
            ...
            return (
              <div key={scorer.name} className="scorer-row">
                ...
                <img src={`/players/${scorer.name.toLowerCase()}.png`} alt={scorer.name} className="scorer-avatar" />
                <span className="scorer-name">{scorer.name}</span>
                <div className="scorer-bar-track">
                  <div className="scorer-bar-fill" style={{ width: `${pct}%` }} />
                </div>
