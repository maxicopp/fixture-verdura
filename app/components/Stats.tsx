...
function getPlayerStats(name: string, fixture: Round[]): PlayerResult[] {
...
function getResult(r: PlayerResult): string {
...
const getColor = (name: string) => PLAYER_COLORS[name] ?? DEFAULT_COLOR
...
function calcHealth(results: PlayerResult[]): HealthResult {
...
function HealthBadge({ health, size = 'md' }: { health: HealthResult; size?: 'sm' | 'md' }) {
...
export default function Stats({ fixture, standings, disabledPlayers = [] }: StatsProps) {
  ...
  const isDisabled = (name: string) => disabledPlayers.includes(name)
  ...
  const topScorer   = standings.length > 0 ? [...standings].sort((a, b) => b.gf - a.gf)[0] : undefined
  const bestDefense = standings.length > 0 ? [...standings].sort((a, b) => a.gc - b.gc)[0] : undefined

  type BiggestWin = { home: string; away: string; homeGoals: number; awayGoals: number; diff: number }
  const biggestWin: BiggestWin | null = allMatches.reduce<BiggestWin | null>((acc, m) => {
    const diff = Math.abs(m.homeGoals! - m.awayGoals!)
    if (!acc || diff > acc.diff) return { home: m.home, away: m.away, homeGoals: m.homeGoals!, awayGoals: m.awayGoals!, diff }
    return acc
  }, null)
  ...
  const cumulativeData = roundsPlayed.map((round, ri) => {
    const entry: Record<string, string | number> = { jornada: `F${round.round}` }
  ...
  // Head to head matrix
  const h2h: Record<string, Record<string, { g: number; e: number; p: number }>> = {}
  ...
  return (
    <div className="stats">
        ...
        {allPlayers.map(name => {
          ...
          return (
            <button
              ...
              style={{ '--chip-color': getColor(name) } as React.CSSProperties}
      ...
      <div className="stats-grid">
        ...
        {biggestWin !== null && biggestWin.diff > 0 && (
