...
function calcOdds(homeName: string, awayName: string, fixture: Round[], histStats: HistStats = {}): Odds {
  function score(name: string): number {
    ...
    // ── Current tournament ───────────────────────────────────────────────
    const results: Array<{ gf: number; gc: number }> = []
...
interface FixtureReadOnlyProps {
  fixture: Round[]
  disabledPlayers?: string[]
  histStats?: HistStats
}

function FixtureReadOnly({ fixture, disabledPlayers = [], histStats = {} }: FixtureReadOnlyProps) {
  const [expandedRound, setExpandedRound] = useState(0)
  const isDisabled      = (name: string)  => disabledPlayers.includes(name)
  const matchHasDisabled = (match: Match) => isDisabled(match.home) || isDisabled(match.away)
  return (
