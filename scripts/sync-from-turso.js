...
async function run() {
  ...
  await local.executeMultiple(`
    CREATE TABLE IF NOT EXISTS tournaments (
      ...
      year INTEGER NOT NULL, status TEXT NOT NULL DEFAULT 'active', champion TEXT,
    ...
    CREATE TABLE IF NOT EXISTS tournament_players (
      ...
      disabled INTEGER DEFAULT 0, UNIQUE(tournament_id, name)
    );
    CREATE TABLE IF NOT EXISTS matches (
      id INTEGER PRIMARY KEY AUTOINCREMENT, tournament_id INTEGER NOT NULL,
      match_key TEXT NOT NULL, round INTEGER NOT NULL, home TEXT NOT NULL, away TEXT NOT NULL,
  ...
  for (const t of tournaments) {
    await local.execute({
      sql: `INSERT INTO tournaments (id, name, season, year, status, champion, top_scorer, top_scorer_goals, created_at, finished_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [t.id, t.name, t.season, t.year, t.status, t.champion ?? null, t.top_scorer ?? null, t.top_scorer_goals ?? 0, t.created_at ?? null, t.finished_at ?? null],
    })
  }
  ...
  for (const p of players) {
    await local.execute({
      sql: `INSERT INTO tournament_players (id, tournament_id, name, disabled) VALUES (?, ?, ?, ?)`,
      args: [p.id, p.tournament_id, p.name, p.disabled ?? 0],
    })
  }
  ...
  for (const m of matches) {
    await local.execute({
      sql: `INSERT INTO matches (id, tournament_id, match_key, round, home, away, home_goals, away_goals, played) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [m.id, m.tournament_id, m.match_key, m.round, m.home, m.away, m.home_goals ?? null, m.away_goals ?? null, m.played ?? 0],
    })
  }
